...
void annexCode(void)
{
    ...
    if (sensors(SENSOR_GPS)) {                 // LED2 for number of sattelites indication
      ...
      if(currentTime > GPSLEDTime) {           // - Fix and sat no. >= 5 -> LED blinks, one blink for 5 sat, two blinks for 6 sat, three for 7 ...
        if(f.GPS_FIX && GPS_numSat >= 5) {
          ...
          GPSLEDTime = currentTime + 150000;
          if(blcnt >= 10 && ((blcnt%2) == 0)) {LED3_ON;} else {LED3_OFF;}
        }else{
          if((GPS_update == 1) && !f.GPS_FIX) {LED3_ON;} else {LED3_OFF;}
          blcnt = 0;
        }
      }
    }
